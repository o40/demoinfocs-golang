...
type Player struct {
	...
	tickRate   float64 // in-game tick rate, used for IsBlinded()
	ingameTick *int    // reference to GameState.inagameTick, used for IsBlinded()
}
...
func (p *Player) FlashDurationTimeRemaining() time.Duration {
	timeSinceFlash := time.Duration(float64(*p.ingameTick-p.FlashTick) / p.tickRate * float64(time.Second))
...
func NewPlayer(tickRate float64, ingameTick *int) *Player {
	return &Player{
